...
export default function PPCChannel({ years }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <MetricStrip
        metrics={[
          { label: 'HVAC Jobs/Month', value: y1.homeServices.energyGrants.hvacJobs, format: 'number' },
          { label: 'Insulation Jobs/Month', value: y1.homeServices.energyGrants.insulationJobs, format: 'number' },
          { label: 'Water Heater Jobs/Month', value: y1.homeServices.energyGrants.waterHeaterJobs, format: 'number' },
          { label: 'Annual Profit (Y1)', value: y1.homeServices.energyGrants.totalProfitAnnual, format: 'currency' },
        ]}
        columns={4}
      />
      ...
      <ChartCard title="5-Year Profit Trajectory" subtitle="Annual profit scaling with PPC spend growth">
        <RevenueLineChart
          data={profitByYear}
          lines={[{ key: 'profit', label: 'PPC Profit', color: '#1b4332' }]}
        />
      </ChartCard>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="luxury-card">
          <h4 className="font-display font-semibold text-text-primary text-lg mb-3">Lead Economics</h4>
        ...
        <div className="luxury-card">
          <h4 className="font-display font-semibold text-text-primary text-lg mb-3">Grant + Rebate Integration</h4>
          <p className="text-text-secondary text-sm leading-relaxed">{channel.grantIntegration}</p>
        </div>
      </div>
    </div>
  )
