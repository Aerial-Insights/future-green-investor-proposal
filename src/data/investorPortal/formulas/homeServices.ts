...
function rebateAdjustedProfit(
  ...
  avgRebateCoverage: number
): number {
  const baseCost = revenuePerJob * (1 - baseMargin)
  const rebateOffset = rebateEligRate * avgRebateCoverage * baseCost
  return (revenuePerJob * baseMargin) + rebateOffset
}
...
function calcRoofingUpsells(roofingJobs: number, a: AllAssumptions): UpsellBreakdown {
  ...
  const hvacProfit = hvacJobs * rebateAdjustedProfit(se.hvacRevenuePerJob, se.hvacMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const insulationProfit = insulationJobs * rebateAdjustedProfit(se.insulationRevenuePerJob, se.insulationMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const waterHeaterProfit = waterHeaterJobs * rebateAdjustedProfit(se.waterHeaterRevenuePerJob, se.waterHeaterMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const airSealingProfit = airSealingJobs * rebateAdjustedProfit(se.airSealingRevenuePerJob, se.airSealingMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
...
function calcInsulationUpsells(insulationBaseJobs: number, a: AllAssumptions): number {
  ...
  const u = a.upsell
  const g = a.grants
  const hvac = insulationBaseJobs * u.hvacToRoofing * rebateAdjustedProfit(se.hvacRevenuePerJob, se.hvacMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const waterHeater = insulationBaseJobs * u.hvacToWaterHeater * rebateAdjustedProfit(se.waterHeaterRevenuePerJob, se.waterHeaterMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const airSealing = insulationBaseJobs * u.hvacToAirSealing * rebateAdjustedProfit(se.airSealingRevenuePerJob, se.airSealingMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
...
function calcWaterHeaterUpsells(whBaseJobs: number, a: AllAssumptions): number {
  ...
  const u = a.upsell
  const g = a.grants
  const hvac = whBaseJobs * u.hvacToRoofing * rebateAdjustedProfit(se.hvacRevenuePerJob, se.hvacMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const insulation = whBaseJobs * u.hvacToInsulation * rebateAdjustedProfit(se.insulationRevenuePerJob, se.insulationMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  const airSealing = whBaseJobs * u.hvacToAirSealing * rebateAdjustedProfit(se.airSealingRevenuePerJob, se.airSealingMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
...
export function calcGrantImpact(a: AllAssumptions, year: number): GrantOutput {
  ...
  // Calculate per-service rebate offsets
  const hvacRebateOffset = grants.hvacJobs * 12 * g.rebateEligibilityRate * g.avgRebateCoveragePercent * se.hvacRevenuePerJob * (1 - se.hvacMargin)
  const insulationRebateOffset = grants.insulationJobs * 12 * g.rebateEligibilityRate * g.avgRebateCoveragePercent * se.insulationRevenuePerJob * (1 - se.insulationMargin)
  const waterHeaterRebateOffset = grants.waterHeaterJobs * 12 * g.rebateEligibilityRate * g.avgRebateCoveragePercent * se.waterHeaterRevenuePerJob * (1 - se.waterHeaterMargin)
  const estimatedRebateOffset = hvacRebateOffset + insulationRebateOffset + waterHeaterRebateOffset
  ...
  const adjustedProfit = totalEnergyJobs * (
    rebateAdjustedProfit(se.hvacRevenuePerJob, se.hvacMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent) +
    rebateAdjustedProfit(se.insulationRevenuePerJob, se.insulationMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent) +
    rebateAdjustedProfit(se.waterHeaterRevenuePerJob, se.waterHeaterMargin, g.rebateEligibilityRate, g.avgRebateCoveragePercent)
  ) / 3
  const profitMultiplier = baseProfit > 0 ? adjustedProfit / baseProfit : 1

  return { grantSupportedDeals, effectiveCloseRate, estimatedReimbursement, rebateEligibleDeals, estimatedRebateOffset, profitMultiplier }
}
